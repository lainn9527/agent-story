...
# Per-branch locks for thread-safe advance_world_day
_branch_locks: dict[str, threading.Lock] = {}
_branch_locks_lock = threading.Lock()


def _get_branch_lock(story_id: str, branch_id: str) -> threading.Lock:
    key = f"{story_id}/{branch_id}"
    with _branch_locks_lock:
...
def advance_world_day(story_id: str, branch_id: str, days: float) -> float:
    """Advance world_day by N days (thread-safe). Returns new world_day."""
    if days <= 0:
        return get_world_day(story_id, branch_id)
    with _get_branch_lock(story_id, branch_id):
...
_TIME_VALUE_RE = re.compile(r'[\d.]+')


def parse_time_tag(time_str: str) -> float:
    """Parse a TIME tag body like 'days:3' or 'hours:8' into days.

    Uses regex to extract numeric value (safe against trailing text).
    Returns 0 if unparseable.
    """
    time_str = time_str.strip()
    if "days:" in time_str:
        after = time_str.split("days:", 1)[1]
        m = _TIME_VALUE_RE.search(after)
        if m:
            try:
    ...
    if "hours:" in time_str:
        after = time_str.split("hours:", 1)[1]
        m = _TIME_VALUE_RE.search(after)
        if m:
            try:
                return float(m.group()) / 24
            except ValueError:
                return 0
    return 0
