...
class TestAsyncEventDedup:
    @mock.patch("llm_bridge.call_oneshot")
    def test_duplicate_event_skipped(self, mock_llm, story_id, setup_story):
        """Events with titles already in the DB should be skipped."""
        # Insert existing event
        event_db.insert_event(story_id, {
            "event_type": "伏筆", "title": "已存在事件", "description": "d"
        }, "main")
