...
# Label-free descriptions for the GM context injection.
# These deliberately avoid naming the outcome so the GM cannot echo it.
_OUTCOME_GM_HINTS = {
    "天命": "本回合趨勢極度有利——超乎預期的機緣降臨。但好運未必沒有隱患，功高震主、樹大招風、樂極生悲皆有可能。",
    "順遂": "本回合趨勢偏向有利——事態發展順暢。但順境中可能暗藏伏筆，過於順利意味著某處正在積累風險。",
    "平淡": "本回合趨勢中性——不偏不倚，一切按常理發展。結果完全取決於角色自身的能力、準備和選擇。",
    "波折": "本回合趨勢偏向不利——遇到阻礙或意外轉折。但塞翁失馬焉知非福，挫折可能帶來意外的發現、盟友、或成長契機。",
    "劫數": "本回合趨勢極度不利——重大考驗降臨，處境艱難。但危機往往是轉機的起點，絕境中的突破、因禍得福、置之死地而後生。",
}
...
def format_dice_context(result: dict) -> str:
    ...
    The hint deliberately avoids naming the outcome label (天命/順遂/平淡/波折/劫數)
    so the GM cannot echo it back to the player.
    """
    return (
        f"[命運走向]\n"
        f"{_OUTCOME_GM_HINTS[result['outcome']]}\n"
        f"（這是系統內部提示，僅供你參考以決定敘事走向。\n"
        f"■ 絕對禁止：\n"
        f"  - 不要在回應中出現任何命運系統相關的標籤、標題、或提示語\n"
        f"  - 不要告訴玩家趨勢是有利還是不利\n"
        f"  - 不要出現「命運骰」「命運走向」「判定」「骰面」「趨勢」「有利」「不利」等機制用語\n"
        f"  - 不要出現解釋性語句如「好運降臨」「機緣來了」「命運眷顧」「考驗來了」\n"
        f"  - 直接用故事事件來體現，讓玩家自己感受\n"
        f"■ 核心原則：塞翁失馬，焉知非福\n"
        f"  - 此提示影響的是「事態發展的趨勢」，不是「當前行動的成敗」\n"
        f"  - 有利不代表沒有隱患，不利不代表沒有收穫\n"
        f"  - 用伏筆、意外發現、NPC反應、環境變化等方式自然體現\n"
        f"  - 確定性行為（兌換、購買、查詢）不受此提示影響\n"
        f"■ 行動合理性影響結果：\n"
        f"  - 玩家描述越周全，行動本身越可能成功\n"
        f"  - 玩家描述草率或不合理，即使趨勢有利也不該輕鬆碾壓\n"
        f"■ 演繹自由度：\n"
        f"  - 你可以自由決定行動本身是否成功，此提示影響的是連帶效應和後續發展\n"
        f"  - 例：不利趨勢 + 周全策略 → 策略成功但過程中遇到意外；有利趨勢 + 魯莽行動 → 僥倖沒死但浪費了好機會）"
    )
