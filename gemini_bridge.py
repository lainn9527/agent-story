...
def embed_text(
    ...
    """Embed a single text using Gemini Embedding API. Returns 768-dim vector or None."""
    body = {
    ...
    result, err = _with_key_fallback(gemini_cfg, _do)
    if err:
        log.warning("gemini_bridge: embed_text failed — %s", err)
        return None
...
def embed_texts_batch(
    ...
    """Batch-embed up to 100 texts. Returns list of 768-dim vectors or None."""
    if not texts:
        return []
    body = {
        "requests": [
    ...
    result, err = _with_key_fallback(gemini_cfg, _do)
    if err:
        log.warning("gemini_bridge: embed_texts_batch failed — %s", err)
        return None
